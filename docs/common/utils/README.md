# utils

- [caching.md](caching.md) - `get_cached`, safe attribute caching inside a custom `__getattribute__`.
- [concurrency.md](concurrency.md) - `ThreadLocal`, `ThreadLock`, `ContextLocal`, named singleton factories.
- [error_handling.md](error_handling.md) - `TransformExceptions`, `SuppressAndRun`, `suppress_callable`.
- [functional.md](functional.md) - `noop`, `identity`, `returns`, `raises`, `with_attrs`, `require_exclusive_keys`, `cloned`, `enabled_if`.
- [iterables.md](iterables.md) - `not_none`, `first_of`, `purge_iterable`, `purge_mapping`, `all_combinations`.
- [metaclasses.md](metaclasses.md) - `is_dunder`, `transform` metaclass for hook-driven class bodies.
- [required_attributes.md](required_attributes.md) - `RequiredAttributesMixin`, fail fast on missing subclass attributes.
- [sentinel.md](sentinel.md) - `Sentinel`, unique named "not provided" markers.
- [strings.md](strings.md) - `camel_to_snake`, `snake_to_pascal`, `snake_to_human`.
