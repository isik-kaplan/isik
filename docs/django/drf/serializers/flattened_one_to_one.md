...
- Known non-goal: only per-field validation on the nested serializer carries over (its fields are harvested individually, not run through its own `is_valid()`) - `Meta.validators`/object-level `validate()` on the nested serializer class don't apply. A model-level validator/`clean()` on the related object still applies though, via `full_clean()` inside its own `save()`; any Django `ValidationError` raised there is translated into a DRF one (`django_to_drf_validation_error`) so it comes back as a normal 400 instead of an unhandled 500.
